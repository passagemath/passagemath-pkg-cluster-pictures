...
from sage.arith.misc import Euler_Phi, gcd, jacobi_symbol
...
class BYTree(Graph):
    ...
    def multiway_cuts(self, vertices):
        ...
        for es in Combinations(self.edges(sort=False), len(vertices) - 1):
            D = copy(self)
            D.delete_edges(es)
            if len(set(tuple(D.connected_component_containing_vertex(v,sort=True)) for v in vertices)) == len(vertices):
                yield es
